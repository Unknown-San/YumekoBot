...
def warn(user: User,
    ...
    if user.id in TIGERS:
        if warner:
            message.reply_text("Demons cant be warned.")
        else:
            message.reply_text(
                "Demon triggered an auto warn filter!\n I can't warn tigers but they should avoid abusing this."
            )
        return

    if user.id in WOLVES:
        if warner:
            message.reply_text("XDemons are warn immune.")
        else:
            message.reply_text(
                "XDemon triggered an auto warn filter!\nI can't warn wolves but they should avoid abusing this."
            )
        return
    ...
    if num_warns >= limit:
        ...
        if soft_warn:  # punch
            ...
            reply = (
                f"<code>❕</code><b>Enmu's Punch Event</b>\n"
            ...
            reply = (
                f"<code>❕</code><b>Enmu's Ban Event</b>\n"
                f"<code> </code><b>•  User:</b> {mention_html(user.id, user.first_name)}\n"
                f"<code> </code><b>•  Count:</b> {limit}")
        ...
        reply = (
            f"<code>❕</code><b>Enmu's Warn Event</b>\n"
            f"<code> </code><b>•  User:</b> {mention_html(user.id, user.first_name)}\n"
...
def remove_warn_filter(update: Update, context: CallbackContext):
    ...
    for filt in chat_filters:
        if filt == to_remove:
            ...
            msg.reply_text("Okay Okay My Bad, I'll stop warning people for that.")
            raise DispatcherHandlerStop
