...
def gban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in DEV_USERS:
        message.reply_text(
            "That Demon is part of the SyDeXaS \nI can't act against An Upper Moon Demon ."
        )
        return

    if int(user_id) in DRAGONS:
        message.reply_text(
            "I spy, with my little eye... Lower Moon Level! Why are you guys turning on each other?"
        )
        return

    if int(user_id) in DEMONS:
        message.reply_text(
            "OOOH someone's trying to gban a XLower Moon Level! *grabs popcorn*"
        )
        return
    ...
    if user_chat.type != "private":
        message.reply_text("That's not a Human!")
        return
